...
- **Multi-Step Registration (November 2025)**: Registration redesigned as a 5-step progressive flow with visual progress indicator and step-by-step validation. Steps: (1) Email/Password, (2) Group Info (name & size), (3) School & Bio (optional), (4) Preferences (optional), (5) Profile Photo (required). Required fields: email, password, group name, group size, profile photo. Optional: school, bio, preferences, gallery photos. No password minimum length. Form only submits when user explicitly clicks final button on step 5. All placeholder text removed for cleaner interface.
